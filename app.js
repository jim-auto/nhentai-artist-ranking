...
function render() {
  ...
  $('#table').innerHTML = rows.slice(0, 250).map((x, i) => `<tr><td class="rank">${i + 1}</td><td class="artist"><a href="${escapeAttr(x.artist_url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(x.artist)}</a></td><td class="score">${fmt(x.favorites)}</td><td>${fmt(x.median_favorites)}</td><td>${fmt(x.top_favorites)}</td><td>${fmt(x.galleries)}</td></tr>`).join('');
